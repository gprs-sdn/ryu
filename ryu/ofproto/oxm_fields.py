...
def generate(modname):
    ...
    for i in mod.oxm_types:
        ...
        oxm_class = i.num >> 7
        if oxm_class != OFPXMC_OPENFLOW_GPRS:
            continue
        ofpxmt = i.num & 0x3f
        td = i.type
        add_attr('OFPXMT_GPRS_' + uk, ofpxmt)
        add_attr('OXM_GPRS_' + uk, mod.oxm_tlv_header(ofpxmt, td.size))
        add_attr('OXM_GPRS_' + uk + '_W', mod.oxm_tlv_header_w(ofpxmt, td.size))
